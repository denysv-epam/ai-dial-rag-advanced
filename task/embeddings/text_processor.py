...
class TextProcessor:
    ...
    def process_text_file(
        ...
        # Resolve file path: prefer path relative to this module when a bare filename is provided
        file_path = file_name
        if not os.path.isabs(file_name):
            module_dir = os.path.dirname(__file__)
            candidate = os.path.join(module_dir, file_name)
            file_path = candidate if os.path.exists(candidate) else file_name

        with open(file_path, "r", encoding="utf-8") as file:
            content = file.read()

        chunks: list[str] = chunk_text(content, chunk_size, overlap)
        embeddings: dict[int, list[float]] = self.embeddings_client.get_embeddings(
        ...
        for i in range(len(chunks)):
            self._save_chunk(embeddings.get(i), chunks[i], file_path)
    ...
    def _save_chunk(self, embedding: list[float], chunk: str, document_name: str):
        ...
        # Ensure document_name fits DB constraints: store only the basename
        db_document_name = os.path.basename(document_name)

        with self._get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    "INSERT INTO vectors (document_name, text, embedding) VALUES (%s, %s, %s::vector)",
                    (db_document_name, chunk, vector_string),
                )
                conn.commit()

        print(f"Stored chunk from document: {db_document_name}")
    ...
    def search(
        ...
        query_embedding = self.embeddings_client.get_embeddings(
            inputs=user_request, dimensions=dimensions
        )[0]
